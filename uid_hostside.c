...
int main(int argc, char ** argv){
    ...
    if(argc != 4){
        printf("UID_hostside > Usage(for UIDwrite): $ ./UID_hostside.exe COM<port_number> <baud_rate> W\n");
        printf("UID_hostside > Usage(for UIDread): $ ./UID_hostside.exe COM<port_number> <baud_rate> R\n");
        exit(-1);
    }
    ...
    GetLocalTime(&t); // or GetLocalTime(&t)
    ...
    if(num_inputs != 16){
      ...
      /*printf("\nThe datetime on devboard was: \nYear:\t%u\nMonth:\t%u\nDate:\t%u\nhh:mm:ss > %02u:%02u:%02u\n", 
                                        datetime_data[0],
                                    datetime_data[1],
                                    datetime_data[2],
                                    datetime_data[3],
                                    datetime_data[4],
                                    datetime_data[5]);
        printf("uuid_partA.testline: %u\n", datetime_data[0]);
        printf("uuid_partA.device_charax: %u\n", datetime_data[1]);
        printf("uuid_partA.manuf_year: %u\n", datetime_data[2]);
        printf("uuid_partA.manuf_month: %u\n", datetime_data[3]);
        printf("uuid_partA.manuf_week: %u\n", datetime_data[4]);
        printf("uuid_partB.test_year: %u\n", datetime_data[5]);
        printf("uuid_partB.test_month: %u\n", datetime_data[6]);
        printf("uuid_partB.test_date: %u\n", datetime_data[7]);*/
